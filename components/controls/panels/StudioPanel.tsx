...
export const StudioPanel: React.FC = () => {
  ...
  // AI Context
  const { apiKeys, showLyrics, setShowLyrics, lyricsStyle, setLyricsStyle } = useAI();
  ...
  const lyricsStylesTranslations = t?.lyricsStyles || {};
  ...
  return (
    <>
      ...
      <div className="p-3 pt-4 h-full flex flex-col border-b lg:border-b-0 lg:border-e border-white/5 overflow-hidden">
         <div className="space-y-4 h-full overflow-y-auto custom-scrollbar pr-1.5">
                     ...
                     <div className="space-y-3 animate-fade-in-up">
                         ...
                         {(fileName || currentSong) && (
                            <div className="bg-white/5 p-3 rounded-lg border border-white/10 space-y-3">
                                ...
                                {isPlaylistOpen && (
                                    <div className="bg-black/20 rounded-lg p-1 max-h-[120px] overflow-y-auto custom-scrollbar space-y-0.5 my-2 border border-white/5 animate-fade-in-up">
                                        {playlist.length === 0 ? (
                                ...
                                <div className="flex items-center justify-center gap-3">
                                    <button onClick={playPrev} className="p-2 text-white/50 hover:text-white hover:bg-white/10 rounded-full transition-all"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M8.445 14.832A1 1 0 0010 14v-2.798l5.445 3.63A1 1 0 0017 14V6a1 1 0 00-1.555-.832L10 8.798V6a1 1 0 00-1.555-.832l-6 4a1 1 0 000 1.664l6 4z" /></svg></button>
                                    ...
                                    <button onClick={playNext} className="p-2 text-white/50 hover:text-white hover:bg-white/10 rounded-full transition-all"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M4.555 5.168A1 1 0 003 6v8a1 1 0 001.555.832L10 11.202V14a1 1 0 001.555.832l6-4a1 1 0 000-1.664l-6-4A1 1 0 0010 6v2.798l-5.445-3.63z" /></svg></button>
                                </div>
                            </div>
                         )}
                     </div>
             ...
             <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5 space-y-3">
                <span className="text-xs font-black uppercase text-white/50 tracking-widest block ml-1">AUDIO PROCESSING</span>
      ...
      {/* Col 2: Display Settings Only (Reduced visual noise) */}
      <div className="p-3 pt-4 h-full flex flex-col border-b lg:border-b-0 lg:border-e border-white/5 overflow-hidden">
        <div className="space-y-4 overflow-y-auto custom-scrollbar pr-1.5 h-full">
            {/* Display Settings Block */}
            <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5 space-y-3">
                <span className="text-xs font-black uppercase text-white/50 tracking-widest block ml-1">{t?.displaySettings || "Display Elements"}</span>
                ...
                <SettingsToggle 
                    ...
                    hintText={audioHints?.albumArtBackground}
                >
                    <div className="pt-1">
                        <Slider 
                ...
                <SettingsToggle 
                    ...
                    hintText={audioHints?.overlayCover}
                />
                ...
                <SettingsToggle 
                    ...
                    hintText={audioHints?.lyrics}
                >
                    <div className="pt-3 space-y-3 animate-fade-in-up border-t border-white/5 mt-1">
                         <CustomSelect 
                              label={t?.lyrics || "Style"} 
                              value={lyricsStyle} 
                              options={Object.values(LyricsStyle).map(s => ({ value: s, label: lyricsStylesTranslations[s] || s }))} 
                         ...
                         <CustomSelect 
                              ...
                              onChange={(val) => setSettings(prev => ({...prev, lyricsPosition: val as Position}))} 
                         />
                    </div>
                </SettingsToggle>
            </div>
      ...
      {/* Col 3: Recording Actions & Stream Settings */}
      <div className="p-3 pt-4 h-full flex flex-col overflow-hidden relative">
          ...
          <div className="space-y-4 h-full overflow-y-auto custom-scrollbar pr-1.5">
              ...
              {/* Record Action Card (Redesigned: Horizontal Layout) */}
              <div className="bg-white/[0.02] rounded-xl p-4 border border-white/5 flex flex-row items-center gap-4">
                  
                  {/* LEFT: Main Action & Status (Flex 1 to take remaining space) */}
                  <div className="flex-1 flex flex-col items-center justify-center gap-2">
                      ...
                      <div className="w-full text-center space-y-1">
                         {isRecording ? (
                             <div className="flex flex-col items-center animate-pulse"><span className="text-lg font-black text-red-400 font-mono">{formatDuration(recordingDuration)}</span><span className="text-[10px] font-bold text-red-400/60">{formatSize(size)}</span></div>
                         ) : (
                             <>
                  ...
                  {/* RIGHT: Toggles (Vertical Stack) */}
                  <div className="flex flex-col gap-2 w-28 shrink-0 border-l border-white/5 pl-4 py-1">
                      <TooltipArea text={hints.syncStart}>
                        <button 
                            onClick={() => setSyncStart(!syncStart)} 
                            className={`w-full relative py-2 px-2 rounded-lg border flex items-center justify-center gap-1.5 transition-all group ${syncStart ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-white/5 border-white/5 text-white/40 hover:bg-white/10 hover:text-white'}`}
                        >
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-wider">{settingsLabels.syncStart || "Sync"}</span>
                        </button>
                      </TooltipArea>
                      ...
                      <TooltipArea text={hints.countdown}>
                        <button 
                            ...
                            className={`w-full relative py-2 px-2 rounded-lg border flex items-center justify-center gap-1.5 transition-all group ${enableCountdown ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' : 'bg-white/5 border-white/5 text-white/40 hover:bg-white/10 hover:text-white'}`}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            <span className="text-[9px] font-bold uppercase tracking-wider">{settingsLabels.countdown || "Timer"}</span>
                            {enableCountdown && <span className="absolute -top-0.5 -right-0.5 flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-blue-500"></span></span>}
                        </button>
                      </TooltipArea>
                  </div>
              </div>
              ...
              {/* Video Stream Config */}
              <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5 space-y-3">
                  <span className="text-xs font-black uppercase text-white/50 tracking-widest block ml-1">{studio.videoStream || "VIDEO STREAM"}</span>
              ...
              {/* Audio Stream Config */}
              <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5 space-y-3">
                  <span className="text-xs font-black uppercase text-white/50 tracking-widest block ml-1">{studio.audioStream || "AUDIO STREAM"}</span>
