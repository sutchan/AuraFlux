...
export const BottomBar: React.FC<BottomBarProps> = ({ isExpanded, setIsExpanded, toggleFullscreen, isIdle }) => {
  ...
  if (sourceType === 'FILE' && fileStatus === 'ready') {
      ...
      return (
        <>
            ...
            {/* Playlist Popup */}
            {showPlaylist && (
                <div ref={playlistRef} className="fixed bottom-28 left-1/2 -translate-x-1/2 w-full max-w-[90vw] md:max-w-xl z-[116] bg-[#050505]/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col max-h-[50vh] pointer-events-auto animate-fade-in-up origin-bottom">
            ...
            <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[115] w-full max-w-[90vw] md:max-w-2xl transition-all duration-700 ease-out ${isIdle && !showPlaylist ? 'opacity-0 translate-y-10 pointer-events-none' : 'opacity-100 translate-y-0 pointer-events-auto'}`}>
                <div className="bg-[#0f0f11]/90 backdrop-blur-2xl border border-white/10 rounded-2xl p-3 md:p-4 shadow-[0_8px_32px_rgba(0,0,0,0.6)] flex items-center gap-4 group">
                    
                    {/* Metadata Section (Left) */}
                    <div className="flex items-center gap-3 w-[25%] min-w-0">
                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0 overflow-hidden shadow-inner relative group/cover`}>
                            {cover ? (
                        ...
                        <div className="flex flex-col min-w-0 hidden sm:flex">
                            <span className="text-xs font-bold text-white truncate leading-tight" title={title}>{title}</span>
                            <span className="text-[10px] text-white/50 truncate font-medium" title={artist}>{artist}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center gap-2">
                        ...
                        <div className="w-full flex items-center gap-2 group/seek">
                            <span className="text-[9px] font-mono text-white/30 w-8 text-right tabular-nums">{formatTime(displayTime)}</span>
                            ...
                            <span className="text-[9px] font-mono text-white/30 w-8 tabular-nums">{formatTime(duration)}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 w-[35%] justify-end">
                        <TooltipArea text={`Mode: ${playbackMode}`}>
                            <button onClick={toggleMode} className={`p-1.5 rounded-lg transition-colors ${playbackMode === 'shuffle' ? 'text-blue-400 bg-blue-400/10' : (playbackMode === 'repeat-one' ? 'text-green-400 bg-green-400/10' : 'text-white/40 hover:text-white hover:bg-white/5')}`}>
                        ...
                        <TooltipArea text={t?.common?.queue || "Queue"}>
                            <button onClick={() => setShowPlaylist(!showPlaylist)} className={`p-1.5 rounded-lg transition-colors ${showPlaylist ? 'text-white bg-white/10' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
                        ...
                        {/* Merged System Controls */}
                        <TooltipArea text={`${t?.hints?.randomize || "Randomize"} [R]`}>
                            <button onClick={randomizeSettings} className="hidden sm:flex p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors">
                        ...
                        <TooltipArea text={`${t?.hints?.fullscreen || "Fullscreen"} [F]`}>
                            <button onClick={toggleFullscreen} className="hidden sm:flex p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors">
                        ...
                        <TooltipArea text={`${t?.showOptions || "Expand"} [H]`}>
                            <button onClick={() => setIsExpanded(true)} className="p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors">
  ...
  return (
    <>
      {renderAiIndicator()}
      <div className={`fixed bottom-8 left-0 w-full z-[110] flex justify-center pointer-events-none px-6 transition-all duration-700 ease-out ${isIdle ? 'opacity-0 translate-y-10' : 'opacity-100 translate-y-0'}`}>
        <div className="flex items-center gap-1 bg-[#0a0a0c]/80 backdrop-blur-xl border border-white/10 rounded-full p-1.5 pl-2 shadow-[0_8px_32px_rgba(0,0,0,0.6)] pointer-events-auto transform hover:scale-[1.02] transition-transform duration-300">
          ...
          <TooltipArea text={`${isListening ? t?.stopMic : t?.startMic} [Space]`}>
            <button
              ...
              className={`w-10 h-10 md:w-11 md:h-11 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 group relative overflow-hidden ${isListening ? 'bg-white text-black shadow-[0_0_20px_rgba(255,255,255,0.3)]' : 'bg-white/10 text-white/60 hover:text-white hover:bg-white/20'}`}>
          ...
          <div className="flex items-center gap-0.5">
            <TooltipArea text={`${t?.hints?.randomize || "Randomize"} [R]`}>
                <button onClick={randomizeSettings} aria-label={t?.randomize} className="w-9 h-9 flex items-center justify-center rounded-full text-white/50 hover:text-white hover:bg-white/10 transition-all active:scale-90">
                    {/* D6 Dice Icon for Visual Randomization */}
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                    </svg>
            ...
            <TooltipArea text={`${t?.hints?.fullscreen || "Fullscreen"} [F]`}>
                <button onClick={toggleFullscreen} aria-label={t?.hints?.fullscreen} className="w-9 h-9 flex items-center justify-center rounded-full text-white/50 hover:text-white hover:bg-white/10 transition-all active:scale-90">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M20 8V4m0 0h-4M4 16v4m0 0h4M20 16v4m0 0h-4" /></svg>
                </button>
            </TooltipArea>
          </div>
          ...
          {/* Expand Button */}
          <TooltipArea text={`${t?.showOptions || "Expand"} [H]`}>
            <button onClick={() => setIsExpanded(true)} aria-label={t?.showOptions} className="h-9 px-4 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 text-white/70 hover:text-white transition-all flex items-center gap-2 active:scale-95 group">
